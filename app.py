...
def health():
    st.title("HEALTH MANAGEMENT APPLICATION")
...
def generate_response(image_b64):
    ...
    llm = ChatNVIDIA(model="nvidia/neva-22b",api_key = api_key)
    response = llm.invoke(
        [
            HumanMessage(
                content=[
...
selected=option_menu(
    menu_title="Menu",
    options=["Search Engine","Document Q&A Application","Document Summarizer Application","IMAGE DETECTOR","Resume Application Tracking System","YouTube Transcript Generator","Health Management Application"],
...
if selected=="Resume Application Tracking System":
    resume_ATS()
if selected=="YouTube Transcript Generator":
    yt_transcript()
if selected=="Health Management Application":
    health()
if selected=="IMAGE DETECTOR":
    img_detector()